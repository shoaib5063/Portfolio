...
const SingleProject = ({ project }) => {
  ...
  return (
    <div className='group w-full h-fit flex flex-col items-center justify-center relative cursor-pointer overflow-hidden px-3 md:px-8 py-[1.4rem] shadow-2xl rounded-lg border' style={{background: 'linear-gradient(90deg, #0b1a13 0%, #070d0b 100%)', borderColor: '#1a3d2b'}}
    >
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        {demo || code ? (
          <a href={demo || code} target='_blank' rel='noopener noreferrer' className="flex flex-col items-center w-full group/link">
            <h2 className='text-[#EFF3F4] not-italic font-semibold text-[1.525rem] leading-[110%] text-center capitalize transition-colors duration-300 group-hover/link:text-[#00ff88]'>
              {name}
            </h2>
            <div className="p-6">
              <Image
        ...
        <div className="flex items-center justify-between w-full">
          {demo && (
            <a
              ...
              className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#00ff88] text-[#00ff88] transition-all duration-300 decoration-clone cursor-pointer no-underline delay-[0.3s] hover:bg-[#00ff88] hover:text-[#070d0b]">
              <FaPlay />
            </a>
          )}
          ...
          {code && (
            <a
              ...
              target='_blank'
              rel='noopener noreferrer'
              className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#00ff88] text-[#00ff88] transition-all duration-300 cursor-pointer no-underline delay-[0.3s] group-hover:translate-x-[-140px] hover:bg-[#00ff88] hover:text-[#070d0b]">
              <FaCode />
            </a>
          )}
        </div>
      </div>
      <p className="absolute w-[90%] md:w-[85%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[110%] rounded-[0_20px_20px_0] left-0 top-0 text-[#EFF3F4] translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-xs md:text-sm border-r border-y" style={{background: '#0b1a13', borderColor: '#00ff88'}}>
        {description}
      </p>
      <div className='group-hover:translate-x-0 absolute w-[140px] text-[0.8rem] flex justify-center gap-2 flex-col translate-x-full transition-transform duration-[0.5s] delay-[0.3s] p-[0.825rem] rounded-[10px_0_0_10px] right-0 bottom-4 text-[#00ff88] border-l border-y' style={{background: '#0b1a13', borderColor: '#1a3d2b'}}>
        {tools && tools.map((tag, id) => (
